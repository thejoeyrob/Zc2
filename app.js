...
function authFormFields(){
  const GOOGLE_G='<svg viewBox="0 0 18 18" width="18" height="18"><path fill="#4285F4" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.874 2.684-6.615z"/><path fill="#34A853" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z"/><path fill="#FBBC05" d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"/><path fill="#EA4335" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z"/></svg>';
  const p=state.oauthProviders;const socials=[p.google&&['google','Google',GOOGLE_G],p.apple&&['apple','Apple','●'],p.discord&&['discord','Discord','D'],p.twitter&&['twitter','X','X']].filter(Boolean);
  const socialHtml=socials.length?`<div class="social-login-grid">${socials.map(([provider,label,mark])=>`<button class="social-login ${provider}" data-oauth="${provider}" type="button"><span class="social-mark">${mark}</span><span>${provider==='google'?'Sign in with Google':'Continue with '+label}</span></button>`).join('')}</div><div class="auth-divider"><span>or use email</span></div>`:'';
...
function bindAuthModal(){
  ...
  $('#authSignInTab').onclick=()=>setMode('signin');$('#authSignUpTab').onclick=()=>setMode('signup');$('#authForm')?.addEventListener('submit',e=>{e.preventDefault();submitAuth();});if(forgot)forgot.onclick=requestPasswordReset;$$('[data-oauth]').forEach(button=>button.onclick=()=>signInWithOAuthProvider(button.dataset.oauth));
}
...
async function submitAuth(){
  ...
  if(!email||password.length<8)return setToast('Enter your email and a password of at least 8 characters.');if($('#rememberEmail')?.checked)localStorage.setItem(REMEMBER_EMAIL_KEY,email);else localStorage.removeItem(REMEMBER_EMAIL_KEY);let res;
  if(mode==='signup'){
...
async function sendCallSignal(to,type,payload={}){
  ...
  const body={room_slug:`call:${id}`,from_user:state.session.user.id,to_user:to,signal:{type,call_id:id,from_name:state.profile?.gamertag||'Player',...payload}};
  let {error}=await supabase.from('voice_signals').insert(body);
  if(error){console.error('Call signal:',error);await new Promise(r=>setTimeout(r,350));const retry=await supabase.from('voice_signals').insert(body);error=retry.error;}
  if(error){console.error('Call signal retry:',error);return false;}return true;
}
function subscribeCallSignals(){
  ...
  state.callSignalPoll=setInterval(()=>pollCallSignals(uid),800);pollCallSignals(uid);
}
async function pollCallSignals(uid){if(!state.session||state.session.user.id!==uid)return;const since=new Date(Date.now()-300000).toISOString();const {data,error}=await supabase.from('voice_signals').select('id,room_slug,from_user,to_user,signal,created_at').eq('to_user',uid).gte('created_at',since).order('created_at',{ascending:true}).limit(80);if(error||!data)return;for(const row of data){if(state.callSeenSignals.has(row.id))continue;state.callSeenSignals.add(row.id);await handleCallSignalRow(row);}if(state.callSeenSignals.size>900)state.callSeenSignals=new Set([...state.callSeenSignals].slice(-450));}
async function recoverRecentCallSignals(){
  if(!state.session||state.call.status!=='idle'||state.groupCall.status!=='idle')return;const since=new Date(Date.now()-90000).toISOString();
  const {data,error}=await supabase.from('voice_signals').select('id,room_slug,from_user,to_user,signal,created_at').eq('to_user',state.session.user.id).gte('created_at',since).order('created_at',{ascending:false}).limit(60);if(error||!data)return;
...
async function startDirectCall(peerId,peerName='Player'){
  ...
  state.call.timeout=setTimeout(()=>{if(state.call.id===id&&state.call.status==='ringing'){const peer=state.call.peerId;endCallLocal('no-answer',true);sendCallSignal(peer,'hangup',{call_id:id,reason:'no-answer'});setToast('No answer.');}},40000);
}
...
function endCallLocal(reason='ended',rerender=true){clearTimeout(state.call.timeout);clearTimeout(state.call.disconnectTimer);resetCallState();if(rerender)render();}
...
function makeDirectCallPc(){
  if(state.call.pc)return state.call.pc;const pc=new RTCPeerConnection(rtcConfig());state.call.stream?.getTracks().forEach(track=>pc.addTrack(track,state.call.stream));pc.onicecandidate=e=>{if(e.candidate)sendCallSignal(state.call.peerId,'ice',{call_id:state.call.id,candidate:e.candidate.toJSON?.()||e.candidate});};pc.ontrack=e=>{let a=state.call.audio;if(!a){a=document.createElement('audio');a.autoplay=true;a.playsInline=true;a.className='remote-call-audio';document.body.appendChild(a);state.call.audio=a;}a.srcObject=e.streams[0];a.play?.().catch(()=>{});if(state.call.status!=='active'){clearTimeout(state.call.disconnectTimer);state.call.status='active';if(!state.call.startedAt)state.call.startedAt=Date.now();render();startCallTimer();}};pc.onconnectionstatechange=()=>{if(pc.connectionState==='connected'){clearTimeout(state.call.disconnectTimer);state.call.status='active';if(!state.call.startedAt)state.call.startedAt=Date.now();render();startCallTimer();}else if(pc.connectionState==='failed'){endCallLocal('failed',true);setToast('Call connection failed.');}else if(pc.connectionState==='disconnected'){clearTimeout(state.call.disconnectTimer);state.call.disconnectTimer=setTimeout(()=>{if(state.call.pc===pc&&pc.connectionState==='disconnected'){endCallLocal('disconnected',true);setToast('Call disconnected.');}},7000);}else if(pc.connectionState==='closed'){endCallLocal('closed',true);}};pc.oniceconnectionstatechange=()=>{if(pc.iceConnectionState==='failed'){endCallLocal('failed',true);setToast('Call connection failed.');}};state.call.pc=pc;return pc;
}
