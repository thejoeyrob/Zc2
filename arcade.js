...
async function open(){await syncProfile();A.adminAtOpen=adminUnlocked();A.open=true;A.menu=false;A.paused=false;document.documentElement.classList.add('zs52-locked');document.body.classList.add('zs52-playing');try{await screen.orientation?.lock?.('any')}catch{}try{if(!document.fullscreenElement&&document.documentElement.requestFullscreen)await document.documentElement.requestFullscreen()}catch{}if(!restoreRun())newRun();renderOverlay();if(A.engine?.started)startLoop();}
...
function renderOverlay(){let r=$('#zs52Root');if(!r){r=document.createElement('div');r.id='zs52Root';document.body.appendChild(r)}r.innerHTML=consoleHtml();bindOverlay();resizeCanvas();draw();hud();}
